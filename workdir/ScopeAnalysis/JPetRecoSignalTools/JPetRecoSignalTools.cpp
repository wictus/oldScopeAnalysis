	#include "./JPetRecoSignalTools.h"
	  ...
	  TFile *outFile = new TFile(fileName.c_str(), "update");  //plik do historyjek
...
// 	  TLine *line = new TLine( -50, -100, 30, -100);
	...
	void JPetRecoSignalTools::savePNGwithMarkedOffsetsAndStartingPoints(const JPetRecoSignal& signal, int number)
	{
	  ...
	  badSignal->GetXaxis()->SetRangeUser(-15,25);
	  badSignal->Draw("AP");
...
/*	  for(unsigned int i = 0; i <5 ; i++)
	  {
		double threshold = -100 - (int)i*50;
	  	TLine *line = new TLine( -5, threshold + offset, signal.getRecoTimeAtThreshold(threshold * -1.0)/1000, threshold + offset);
		  line->SetLineWidth(4);
		  line->SetLineColor(3);
		  line->Draw();
	  }
	  ...
	  for(unsigned int i = 0; i <5; i++)
          {
                double threshold = -100 - (int)i*50;
                TLine *line = new TLine(  signal.getRecoTimeAtThreshold(threshold * -1.0)/1000, threshold + offset, signal.getRecoTimeAtThreshold(threshold * -1.0)/1000, -500);
                  line->SetLineWidth(4);
                  line->SetLineColor(2);
                  line->Draw();
          }
*/
//Plotting thresholds in time domain
/*
          for(unsigned int i = 0; i <20; i+=2)
          {
                TLine *line = new TLine( -5, amplitudePoints[240+i]+offset, signalPoints[240+i].time / 1000.0, amplitudePoints[240+i]+offset);
                  line->SetLineWidth(4);
                  line->SetLineColor(2);
                  line->Draw();
          }
	...
	for(unsigned int i = 0; i <20; i+=2)
	  {
                TLine *line = new TLine( signalPoints[240+i].time / 1000 , -500, signalPoints[240+i].time / 1000, amplitudePoints[240+i] + offset);
		  line->SetLineWidth(4);
		  line->SetLineColor(3);
		  line->Draw();
	  }
  ...
  TLine *offsetLine2= new TLine(signalPoints[0].time/1000,offset,signalPoints[startingIndex].time/1000,offset);
	...
	double JPetRecoSignalTools::calculateTOT(const JPetRecoSignal& signal, const double threshold)
	{
	  if(signal.getOffset() == JPetRecoSignalTools::ERRORS::badOffset)
		...
		return calculateConstantThresholdOnFallingEdge(signal, -1*threshold) - calculateConstantThreshold(signal, -1.0*threshold);
	}


	double JPetRecoSignalTools::calculateConstantThresholdOnFallingEdge(const JPetRecoSignal& signal, const double threshold)
	{
  		const std::vector< shapePoint > signalPoints = signal.getShape();
		...
		double timeAtThr= JPetRecoSignalTools::ERRORS::badTimeAtThr;
...
void JPetRecoSignalTools::saveTH1FsToRootFile(std::vector<TH1F*> histoCollection, std::string fileName, std::string pdfName)
{
	...
	//~ testFile->mkdir(directoryName.c_str());
	//~ testFile->cd(directoryName.c_str());
...
{//zwraca najwiekszy histogram w wysokosci
	int max = -100000.;
	int histoMax =0;
	int maxIndex =histoCollection.size()+5;//zeby specjalnie bylo poza zakresem, gdyby byl bug to sie wywali
